...
const saveReminderToDatabase = async (title, description, dateTimeString) => {
  ...
  try {
    const response = await fetch("https://involved-shaylynn-random999-7653907d.koyeb.app/api/reminders", {
      method: "POST",
      headers: {
...
const deleteReminder = async (reminderId, row) => {
  try {
    const response = await fetch(`https://involved-shaylynn-random999-7653907d.koyeb.app/api/reminders/${reminderId}`, {
      method: 'DELETE',
    });
    if (response.ok) {
